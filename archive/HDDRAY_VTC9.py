...
def repair_mode(settings, drive):
    ...
    # Determine drive size for min and max sectors if not set in settings
    if settings['max_sector'] == 0:
        try:
            result = subprocess.run(
                ["powershell", "-Command", f"(Get-Partition -DiskNumber (Get-Disk | Where-Object DeviceID -eq '{drive[4:]}').Number).Size | Measure-Object -Sum | Select-Object -ExpandProperty Sum"],
                capture_output=True,
                text=True,
            )
            drive_size_bytes_str = result.stdout.strip()
            if drive_size_bytes_str == '':
                raise ValueError("PowerShell command returned empty output.")
            drive_size_bytes = int(drive_size_bytes_str)
            max_sector = drive_size_bytes // SECTOR_SIZE
        except Exception as e:
