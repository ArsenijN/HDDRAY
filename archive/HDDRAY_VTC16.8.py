...
def read_settings():
    ...
    settings = {
        'max_latency': int(config['DEFAULT'].get('max_latency', 25)),
        'max_retries': int(config['DEFAULT'].get('max_retries', 8)),
        'max_repair_latency': int(config['DEFAULT'].get('max_repair_latency', 50)),
        'min_sector': int(config['DEFAULT'].get('min_sector', 190086)),
        'max_sector': int(config['DEFAULT'].get('max_sector', 190190)),
        'regenerator_reads': int(config['DEFAULT'].get('regenerator_reads', 16))
    }
    return settings
...
def repair_sector(settings, drive, sector, pattern_cycle, verbose=True):
    ...
    patterns = [b'\x55' * SECTOR_SIZE, b'\xAA' * SECTOR_SIZE, b'\x55' * SECTOR_SIZE, b'\xAA' * SECTOR_SIZE, b'\x55' * SECTOR_SIZE, b'\xAA' * SECTOR_SIZE, b'\x55' * SECTOR_SIZE, b'\xAA' * SECTOR_SIZE, b'\x55' * SECTOR_SIZE, b'\xAA' * SECTOR_SIZE, b'\x55' * SECTOR_SIZE, b'\xAA' * SECTOR_SIZE, b'\x55' * SECTOR_SIZE, b'\xAA' * SECTOR_SIZE, b'\x55' * SECTOR_SIZE, b'\xAA' * SECTOR_SIZE]  # Binary patterns
...
def regenerator_mode(settings, drive):
    ...
    for sector in range(min_sector, max_sector):
        ...
        for read_attempt in range(regenerator_reads):
            #next_sector = sector + 1 if sector + 1 < max_sector else sector - 1
    ...
    for sector in range(min_sector, max_sector):
        ...
        for read_attempt in range(regenerator_reads):
            ...
            if success and latency <= settings['max_repair_latency']:
                ...
                if is_good:
                    break  # Exit the loop if the sector was successfully repaired
                else:
                    current_fail_count += 1
                    if current_fail_count >= settings['max_retries']:
                        print(f"Sector {sector} failed to be repaired after {settings['max_retries']} retries. Skipping sector.")
                        with open(recovered_sectors_file, 'a') as f:
                            f.write(f"{sector} | - | {settings['max_retries']} | 8 | - | - | F | *\n")
                        break  # Exit the loop if the maximum retries have been reached
